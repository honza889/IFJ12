 ...
 */
int FSM(FILE *f, Token *token, char *last_letter, unsigned *line_num)
{
  switch (*last_letter) {
    case '/':
      *last_letter = getc(f);
	...
	if (*last_letter == '\n')
	  line_num++;
	return EXIT_SUCCESS;	// Nahlášení detekce EOF nechám na fci scanner()
      }
      else if (*last_letter == '*')
      {
...
int scanner(FILE *f, Token *token)
{
  ...
  if (isalpha(last_letter)) {
    ...
    else {		// TODO: má se to ukončit nebo to oznámit někde jinde?
      ERROR("scanner: Číslo se nepodařilo převést!");
      exit(1);
    }
  }
  else if (last_letter == '"') {
  ...
  else if (FSM(f, token, &last_letter, &line_num));
  else {		// TODO: má se to ukončit nebo to oznámit někde jinde?
    ERROR("scanner: Nerozpoznan lexem!");
    exit(1);
  }
