...
Value find( ValueList data, int count )
{
    ...
    RCString a = getValueString( testValue(&data[0], typeString, VARIABLE) );
    RCString b = getValueString( testValue(&data[1], typeString, VARIABLE) );
...
{
    RCString str = getValueString(testValue(&data[0], typeString, VARIABLE));
    if (str.length <= 1) {
